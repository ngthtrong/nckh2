...
6. **Chỉ số Tổn thương Nhân khẩu học ($V_i$):** Dựa trên việc phân tích NLP sâu để phát hiện sự xuất hiện của các đối tượng yếu thế thông qua các cụm từ như "có trẻ sơ sinh", "cụ già bị kiệt sức", "phụ nữ mang thai". Biến số này đóng vai trò như một hệ số nhân (multiplier) rủi ro, điều chỉnh sự công bằng trong định tuyến cứu hộ.[^42]
7. **Độ tin cậy của thông tin ($C_i$):** Trong khủng hoảng, hiện tượng tin giả hoặc báo cáo sai lệch thường xuyên xảy ra. Cần thiết lập một hệ số độ tin cậy được tính toán từ lịch sử xác thực của người dùng hoặc sự đồng thuận (consensus) với các cảm biến vật lý lân cận.[^9]
...
Trọng số $w_{ij}$ giữa hai đỉnh sự kiện được tính bằng tổ hợp tuyến tính (hoặc phi tuyến) của ba độ đo tương đồng chính:

$$
w_{ij} = \alpha \cdot \mathcal{S}_{geo}(L_i, L_j) + \beta \cdot \mathcal{S}_{temp}(T_i, T_j) + \gamma \cdot \mathcal{S}_{context}(v_i, v_j)
$$

Trong đó:

* $\mathcal{S}_{geo}(L_i, L_j)$ là hàm tương đồng về không gian. Thay vì sử dụng khoảng cách tuyến tính, hàm suy giảm hàm mũ (Exponential decay) hoặc phân phối Gaussian được áp dụng để trừng phạt mạnh mẽ các khoảng cách lớn:

$$
\mathcal{S}_{geo} = \exp\left( - \frac{\text{dist}(L_i, L_j)^2}{2\sigma_{geo}^2} \right)
$$

  Điều này đảm bảo chỉ những sự kiện ở vùng lân cận thực sự mới có liên kết cấu trúc mạnh mẽ, phù hợp với bán kính hoạt động của một đơn vị ca nô cứu hộ.[^46]

* $\mathcal{S}_{temp}(T_i, T_j)$ là hàm tương đồng về thời gian. Những sự kiện được báo cáo gần nhau về mặt thời điểm sẽ có độ liên kết cao hơn, giúp hệ thống theo dõi được tính động (dynamics) của sự kiện như hướng lũ tràn qua các khu dân cư.[^26]
* $\mathcal{S}_{context}(v_i, v_j)$ là độ tương đồng ngữ cảnh, đại diện cho sự hội tụ của các rủi ro vật lý và nhân khẩu học. Hàm này đo lường sự khác biệt về mức độ ngập lụt $\Delta F = |F_i - F_j|$ và sự tương đồng về mức độ khẩn cấp ($E$). Nếu hai người dùng ở cách nhau 1 km nhưng cùng gửi báo cáo về tình trạng "nước ngập lút mái nhà" ($F \approx 1.0$), cạnh nối giữa họ sẽ được khuếch đại đáng kể để tạo thành một quần thể khẩn cấp. Trái lại, nếu một người dùng báo ngập nhẹ (an toàn ở tầng 3\) và một người báo ngập nặng (đang bám trên mái nhà), trọng số sẽ bị thu hẹp do đặc thù nhu cầu cứu hộ của hai đối tượng là hoàn toàn khác biệt.
* $\alpha, \beta, \gamma$ là các tham số cân bằng, có thể do chuyên gia thiết lập hoặc được học tự động thông qua quá trình tối ưu hóa (Graph Neural Networks).[^9]
...
Thay vì điều phối cảm tính, báo cáo đề xuất một "Hàm Đánh giá Ưu tiên" (Priority Scoring Function) $\mathcal{P}(C_k)$ được thiết lập cho từng cụm $C_k$ sau khi thuật toán Louvain hoàn tất:

$$
\mathcal{P}(C_k) = \omega_1 \cdot \mathcal{E}_{agg}(C_k) + \omega_2 \cdot \mathcal{F}_{max}(C_k) + \omega_3 \cdot \mathcal{N}_{total}(C_k) + \omega_4 \cdot \mathcal{V}_{agg}(C_k)
$$

Chi tiết các thành phần toán học như sau:

* **Điểm Khẩn cấp Trung bình $\mathcal{E}_{agg}(C_k)$**: Bằng $\frac{1}{|C_k|} \sum_{v_i \in C_k} E_i \cdot C_i$. Trung bình mức độ khẩn cấp của các sự kiện trong cụm, có tính đến độ tin cậy của thông tin ($C_i$). Một cụm có nhiều báo cáo nguy cấp từ các nguồn đáng tin cậy sẽ đẩy điểm số này lên cao.
...
* **Quy mô Sinh mạng $\mathcal{N}_{total}(C_k)$**: Bằng $\sum_{v_i \in C_k} N_i$. Tính tổng số lượng sinh mạng đang bị đe dọa. Biến số này tuân thủ nguyên tắc tối đa hóa hiệu suất cứu hộ (Efficiency/Effectiveness) bằng cách hướng nguồn lực đến các khu vực đông người nhất.[^42]
* **Khuếch đại Tổn thương $\mathcal{V}_{agg}(C_k)$**: Bằng $1 + \tanh\left( \sum_{v_i \in C_k} V_i \right)$. Đây là hệ số khuếch đại rủi ro nhân khẩu học. Sử dụng hàm kích hoạt $\tanh$ để chuẩn hóa và giới hạn hệ số này trong một khoảng an toàn (tránh hiện tượng điểm số bùng nổ vô cực), nhưng vẫn đảm bảo tính công bằng (Equity). Sự hiện diện của trẻ em, người bệnh, hoặc người cao tuổi sẽ tạo ra một cú hích lớn về điểm số, đẩy cụm đó lên vị trí ưu tiên cao hơn.[^42]
* Các trọng số quyết định $\omega_1, \dots, \omega_4$ được thiết lập linh hoạt bởi ban chỉ huy thông qua một Ma trận Quyết định (Decision Matrix).[^54] Việc tinh chỉnh các trọng số này cho phép hệ thống chuyển đổi trạng thái chiến thuật (ví dụ: ưu tiên cứu số đông vs. ưu tiên khu vực ngập sâu nhất).
