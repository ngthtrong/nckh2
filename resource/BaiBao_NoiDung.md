...
**Bộ dữ liệu.** Do chưa có bộ dữ liệu thực đã gán nhãn ground-truth cho cụm cứu hộ, chúng tôi sinh một bộ dữ liệu **mô phỏng tất định** (seed = 42) đặc thù cho Miền Trung Việt Nam (Huế – Quảng Trị – Quảng Nam – Đà Nẵng; 15,7–17,1°N, 107,0–108,6°E), gồm **285 sự kiện** trải trên **12 nhãn ground-truth**:

- **240 sự kiện lõi** phân bố quanh **6 "ốc đảo" ngập** (mỗi cụm ~40 điểm, nhãn 0–5).
- **20 sự kiện nhiễu** rải rác (`gt_cluster = -1`), trong đó **5 là tin giả** (`is_fake`).
- **25 sự kiện kịch bản minh họa** — 24 điểm tạo thành **6 nhóm stress-test** (nhãn 100–105) cộng 1 tin giả cô lập (`gt_cluster = -1`). Bốn kịch bản S1–S4, mỗi kịch bản stress-test một quyết định thiết kế:
