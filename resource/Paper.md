...
Để kiểm chứng từng quyết định thiết kế ở Mục 4, chúng tôi hiện thực hóa toàn bộ pipeline bằng Python (`numpy`, `networkx`, `python-louvain`, `igraph`, `leidenalg`, `scikit-learn`, `scipy`; chín thí nghiệm `exp1`–`exp9` trong `demo/experiments/`) và chạy trên một bộ dữ liệu mô phỏng tất định (seed = 42) gồm **285 sự kiện** đặc thù Miền Trung Việt Nam (Huế – Quảng Trị – Quảng Nam – Đà Nẵng): 240 sự kiện lõi quanh 6 "ốc đảo" ngập (có nhãn ground-truth), 20 nhiễu (~40% tin giả), và 25 kịch bản stress-test (S1–S4). Độ đo: ARI/NMI so với ground-truth, đường kính địa lý cụm (km), Modularity $Q$. Tham số mặc định: $\sigma_{geo}=700$ m, $\tau_{temp}=45$ phút, $\tau_F=0{,}25$, $\tau_E=0{,}35$, $\beta=\gamma=0{,}5$, $\theta=0{,}05$, $k=12$, $\lambda=1{,}0$, $s=10$, $\omega=(0{,}34;\,0{,}33;\,0{,}33)$.
