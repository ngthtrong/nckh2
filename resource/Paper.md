...
Để kiểm chứng từng quyết định thiết kế ở Mục 4, chúng tôi hiện thực hóa toàn bộ pipeline bằng Python (`numpy`, `networkx`, `python-louvain`, `igraph`, `leidenalg`, `scikit-learn`, `scipy`; chín thí nghiệm `exp1`–`exp9` trong `demo/experiments/`) và chạy trên một bộ dữ liệu mô phỏng tất định (seed = 42) gồm **285 sự kiện** đặc thù Miền Trung Việt Nam (Huế – Quảng Trị – Quảng Nam – Đà Nẵng) trải trên **12 nhãn ground-truth**: 240 sự kiện lõi quanh 6 "ốc đảo" ngập (nhãn 0–5), 20 nhiễu (`gt=-1`, trong đó 5 tin giả), và 25 kịch bản stress-test (S1–S4) — 24 điểm tạo 6 nhóm (nhãn 100–105, trùng tọa độ ốc đảo nên chặn trần ARI dưới 1,0) cộng 1 tin giả cô lập. Độ đo: ARI/NMI so với ground-truth, đường kính địa lý cụm (km), Modularity $Q$. Tham số mặc định: $\sigma_{geo}=700$ m, $\tau_{temp}=45$ phút, $\tau_F=0{,}25$, $\tau_E=0{,}35$, $\beta=\gamma=0{,}5$, $\theta=0{,}05$, $k=12$, $\lambda=1{,}0$, $s=10$, $\omega=(0{,}34;\,0{,}33;\,0{,}33)$.
...
**Độ đo phân biệt vượt trên ARI (Thí nghiệm 9).** ARI bão hòa ở đỉnh bảng (Louvain, Leiden, Agglomerative, cả HDBSCAN đều ~0,89–0,892), che các khác biệt chất lượng. Bổ sung bộ ba homogeneity/completeness/V-measure phân biệt tốt hơn hẳn ở đúng loại lỗi quan trọng — **completeness** (một sự kiện ground-truth có bị xé lẻ không): Spectral, dù ARI chỉ cách nhóm gần-hòa 0,003, bị phơi bày với completeness chỉ **0,595** (V-measure 0,727) so với Louvain completeness **1,0** (V-measure 0,927); HDBSCAN hòa ARI nhưng tụt còn 0,929 vì gộp các ốc đảo. Độ trải completeness (0,405) tách được các phương pháp đỉnh mà ARI không tách nổi, nên ta báo cáo V-measure kèm ARI xuyên suốt.
