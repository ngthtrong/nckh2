...
**Ghi chú về việc dùng lại $F, E$ ở hai khâu.** Hai thuộc tính $F$ và $E$ xuất hiện cả ở khâu gom cụm (qua $\mathcal{S}_{context}$) lẫn khâu ưu tiên (qua $\mathcal{F}_{max}, \mathcal{E}_{agg}$). Đây không phải double-counting sai vì hai khâu đo hai đại lượng khác bản chất: $\mathcal{S}_{context}$ đo *độ tương đồng* giữa cặp sự kiện (để quyết định chúng có cùng một tình huống hay không), còn $\mathcal{F}_{max}/\mathcal{E}_{agg}$ đo *độ nghiêm trọng tuyệt đối* của cụm (để xếp hạng). Tuy vậy cần thừa nhận: cụm được gom vì $F$ tương đồng thì $\mathcal{F}_{max}$ của nó gần như chắc chắn cao — nên $\mathcal{F}_{max}$ nên hiểu là "mức ngập đặc trưng của một quần thể đã đồng nhất" chứ không phải tín hiệu độc lập hoàn toàn với tiêu chí gom cụm. Thí nghiệm 6 (Mục 4bis) định lượng tính vòng tròn này: loại $\mathcal{S}_{context}$ làm đổi phân cụm (ARI tụt) nhưng gần như không đổi thứ hạng ưu tiên (Kendall's τ = 0,9829), cho thấy $\mathcal{S}_{context}$ chủ yếu hỗ trợ *gom nhóm* chứ không ngấm ngầm quyết định *thứ hạng*.

## **4bis. Kiểm chứng Thực nghiệm**

Để kiểm chứng từng quyết định thiết kế ở Mục 4, chúng tôi hiện thực hóa toàn bộ pipeline bằng Python (`numpy`, `networkx`, `python-louvain`, `igraph`, `leidenalg`, `scikit-learn`, `scipy`; mười thí nghiệm `exp1`–`exp10` trong `demo/experiments/`) và chạy trên một bộ dữ liệu mô phỏng tất định (seed = 42) gồm **285 sự kiện** đặc thù Miền Trung Việt Nam (Huế – Quảng Trị – Quảng Nam – Đà Nẵng) trải trên **12 nhãn ground-truth**: 240 sự kiện lõi quanh 6 "ốc đảo" ngập (nhãn 0–5), 20 nhiễu (`gt=-1`, trong đó 5 tin giả), và 25 kịch bản stress-test (S1–S4) — 24 điểm tạo 6 nhóm (nhãn 100–105, mỗi nhóm neo tại tâm một ốc đảo nên chồng lấn không gian, chặn trần ARI dưới 1,0) cộng 1 tin giả cô lập. Độ đo: ARI/NMI so với ground-truth, đường kính địa lý cụm (km), Modularity $Q$. Tham số mặc định: $\sigma_{geo}=700$ m, $\tau_{temp}=45$ phút, $\tau_F=0{,}25$, $\tau_E=0{,}35$, $\beta=\gamma=0{,}5$, $\theta=0{,}05$, $k=12$, $\lambda=1{,}0$, $s=10$, $\omega=(0{,}34;\,0{,}33;\,0{,}33)$.

**Kiểm chứng công thức (Thí nghiệm 1).** (A) Dạng nhân/gating và dạng cộng cho **cùng ARI 0,892** nhưng đường kính cụm trung bình giảm từ **100,07 km xuống 0,30 km** — địa lý-làm-cổng-chặn co cụm về đúng tầm ca nô mà không hy sinh độ chính xác. (B) Không chuẩn hóa, cụm dân số lớn nhất (216 người) áp đảo bảng xếp hạng; sau chuẩn hóa, cụm có lõi rủi ro cân bằng lên đầu. (C) $\mathcal{V}$ nhân co giãn theo lõi rủi ro (khuếch đại thật) trong khi $\mathcal{V}$ cộng chỉ là offset. (D) $\tanh(\sum V_i)$ bão hòa ($\approx 2$) từ $\sum V_i=3$, còn $\tanh(\sum V_i/10)$ phân biệt được tới $\sum V_i=50$. (E) Gate $C_i$ cho $\mathcal{N}$: tin giả S3 khai 200 người ($C_i=0{,}45$) bị hạ **55%** còn 90. (F) Gate $C_i$ cho $\mathcal{F}_{max}$: cùng tin giả khai $F=0{,}99$ bị hạ từ 0,99 xuống 0,45, khôi phục nhất quán chống tin giả trên cả ba thành phần lõi.

**Độ nhạy (Thí nghiệm 2).** ARI ổn định 0,892 với $\lambda\le1{,}5$; $\sigma_{geo}$ điều khiển đánh đổi đường kính/số cụm (0,28 km ở 200 m → 1,59 km ở 4000 m); $s=10$ cho vùng phân biệt $\mathcal{V}_{agg}$ tốt tới $\sum V_i\approx50$. Trả lời lo ngại "quá nhiều tham số tự do": các độ nhạy ngữ cảnh $\tau_F,\tau_E$ **trơ** với độ chính xác phân cụm (ARI giữ 0,892 trên toàn lưới $[0{,}15;\,0{,}5]$ vì cổng địa lý chi phối cấu trúc cụm); còn cân bằng $\beta/\gamma$ chỉ ảnh hưởng ở cực trị (ARI giữ 0,892 khi $\beta\le0{,}5$ nhưng tụt còn 0,7855 khi $\beta\ge0{,}9$ bỏ đói ngữ cảnh). Nghĩa là chỉ $\lambda,\sigma_{geo},\beta/\gamma$ là các núm tinh chỉnh thực sự liên quan độ chính xác, đều có khoảng an toàn rõ ràng.

**Louvain vs Leiden (Thí nghiệm 3).** Trên 10 seed, cả hai đều cho **0 cộng đồng đứt gãy** (ARI 0,892, $Q=0{,}8311$): chính đồ thị gating đã loại trước rủi ro đứt gãy, nên Leiden là "bảo hiểm miễn phí" chứ không bắt buộc trong bối cảnh này — một kết luận báo cáo trung thực thay vì phóng đại.

**So sánh baseline (Thí nghiệm 4).** Chạy các baseline trên **cùng đồ thị gating** để công bằng: Louvain/Leiden đạt ARI **0,892** (đường kính 0,30 km); Spectral Clustering chỉ 0,339 (đồ thị thưa gây khó phân tách phổ); HDBSCAN 0,890 nhưng chỉ 11 cụm với đường kính 25 km (bị thổi lên do thùng nhiễu + một cụm rải rác, vì nó xé lẻ ốc đảo chứ không gộp); Agglomerative khớp Louvain nhưng cần biết trước $K$. Các baseline hình học trên tọa độ thô kém hơn: K-Means (đúng $K$) 0,688, DBSCAN tốt nhất 0,730. Kết luận: ưu thế đến từ **sự kết hợp** đồ thị gating + tối ưu Modularity (tự tìm $K$, gắn kết không gian), không chỉ từ một thành phần.

**Độ ổn định xếp hạng (Thí nghiệm 5).** Nhiễu loạn $\omega$ quanh mặc định (200 thử nghiệm/mức), đo Kendall's τ của thứ hạng $\mathcal{P}(C_k)$: τ trung bình **0,994** ở ±0,05, **0,986** ở ±0,10, và 0,957 ở ±0,20; tập 3 cụm ưu tiên cao nhất giữ nguyên 99–100% ở dao động thực tế — xếp hạng ổn định, không tùy tiện theo $\omega$. Bổ sung kiểm tra **ổn định cấu trúc** theo $\sigma_{geo}$: với $\sigma_{geo}\in\{400,550,700,900\}$ m phân hoạch giữ nguyên 27 cụm và thứ hạng khớp hoàn hảo (τ = 1,0), chỉ tới 1200 m mới gộp còn 26 cụm (τ = 0,9815) — danh sách ưu tiên bền cả với dao động tham số hình học lẫn trọng số chính sách.

**Ablation tính vòng tròn của ngữ cảnh (Thí nghiệm 6).** Để kiểm tra lo ngại rằng việc dùng lại $F, E$ khiến $\mathcal{S}_{context}$ ngấm ngầm chi phối thứ hạng, ta loại $\mathcal{S}_{context}$ (đặt $\gamma=0$) rồi so cả phân cụm lẫn thứ hạng với mô hình đầy đủ. Bỏ ngữ cảnh **làm đổi phân cụm** (ARI 0,892 → 0,7855; NMI 0,927 → 0,8741; 27 → 30 cụm) nhưng **gần như không đổi thứ hạng**: trên 27 cụm khớp theo trọng tâm, Kendall's τ = **0,9829** và cả top-5 được giữ nguyên. Sự phân ly này chứng minh $\mathcal{S}_{context}$ chủ yếu hỗ trợ *gom nhóm* chứ không chi phối *xếp hạng* — thứ hạng do các số hạng nghiêm-trọng-tuyệt-đối $\mathcal{F}_{max}, \mathcal{E}_{agg}$ dẫn dắt như thiết kế, nên việc dùng lại không phải double-counting độc hại.

**Trọng số tổn thương có giúp người yếu thế? (Thí nghiệm 7).** Câu hỏi sâu của Khe hở 2 không phải "$V_i$ có đổi thứ hạng không" (có, theo thiết kế) mà là nó có tạo **kết quả cứu hộ tốt hơn** cho nhóm yếu thế không. Mô phỏng điều phối rời rạc (3 ca nô, 30 km/h, phục vụ 15 phút/cụm) đo thời gian đến nạn nhân yếu thế theo ba chính sách: nhân đầy đủ, cộng, và mù tổn thương (bỏ $V_i$). Bỏ $V_i$ làm trễ thời gian trung bình đến người yếu thế từ **146,5 lên 163,6 phút — cải thiện 10,4%** nhờ trọng số tổn thương, và giảm thời gian phản ứng có trọng số tổn hại 8,7%. Dạng nhân đổi lấy phần công bằng này bằng chi phí khiêm tốn ở thời gian đến trung bình cho *toàn bộ* nạn nhân (942,9 vs 719,6 phút) — một đánh đổi công bằng–hiệu quả tường minh, không chỉ là xáo thứ hạng.

**Bộ phát hiện tin cậy và độ bền đối kháng (Thí nghiệm 8).** Xem $C_i$ thấp là cờ báo giả trên 285 sự kiện, $C_i$ tách tin giả khỏi tin thật với **ROC-AUC 0,9651** (trung bình 0,50 cho giả vs 0,92 cho thật). Ta cố ý dò điểm gãy: tin giả ngây thơ (cô lập, không ảnh) chỉ đạt $C_i=0,45$; nhưng kẻ tấn công **thêm ảnh giả** đẩy lên 0,77, **dàn dựng corroboration** đẩy lên 0,74, làm **cả hai** đạt 0,92 — không phân biệt được với tin thật. Vì "tính độc lập" của corroboration chỉ xấp xỉ bằng gần kề không gian – thời gian, heuristic bền với spam đơn lẻ nhưng **không** bền với đối thủ phối hợp có nguồn lực. Do đó $C_i$ là bộ lọc tuyến đầu phải kết hợp tin cậy cấp tài khoản/kiểm chứng chéo kênh — giới hạn được nêu rõ thay vì thổi phồng.

**Phân rã chất lượng cụm — completeness vượt trên ARI (Thí nghiệm 9).** ARI bão hòa ở đỉnh bảng (Louvain, Leiden, Agglomerative, cả HDBSCAN đều ~0,89–0,892), che các khác biệt chất lượng. Ta **phân rã** độ khớp thành cặp homogeneity/completeness (trung bình điều hòa của chúng — V-measure — chính bằng NMI trung-bình-cộng đã báo cáo, nên giá trị phân biệt đến từ hai **thành phần** chứ không từ một độ đo tổng hợp mới). Phân rã này phân biệt tốt hơn hẳn ở đúng loại lỗi quan trọng — **completeness** (một sự kiện ground-truth có bị xé lẻ không), phơi bày hai loại lỗi ARI làm mờ. Thứ nhất, ở đầu thấp, Spectral làm vỡ vụn sự kiện: completeness chỉ **0,595** (V-measure 0,727) so với Louvain completeness **1,0** (V-measure 0,927). Thứ hai, và đáng nói hơn, trong bốn phương pháp dẫn đầu mà ARI gộp thành gần-hòa (Louvain, Leiden, Agglomerative, HDBSCAN — chênh nhau trong 0,002 ARI), completeness vẫn tách được: ba phương pháp đầu đạt hoàn hảo 1,0 trong khi HDBSCAN tụt còn **0,929** vì **xé lẻ** sự kiện (đẩy $\sim$36 điểm có nhãn vào thùng nhiễu, tách một ốc đảo ra nhiều cụm) — khoảng cách completeness 0,07 ở nơi ARI chỉ hiện 0,002 (homogeneity của nó là 0,915, còn cao hơn Louvain 0,864, nên lỗi lộ ở completeness chứ không phải homogeneity). Do đó ta báo cáo phân rã homogeneity/completeness kèm ARI xuyên suốt.
