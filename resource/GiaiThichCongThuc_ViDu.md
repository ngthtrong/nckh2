...
- **Đa phương thức tăng độ tin cậy phân loại khủng hoảng.** Mô hình SCBD (SSE-Cross-BERT-DenseNet) và khung CrisisSpot [^11] cho thấy kết hợp bằng chứng ảnh + văn bản qua chú ý chéo (cross-attention) làm tăng F1 5–9,45% so với đơn phương thức. Ta kế thừa nguyên lý "có ảnh xác thực ⇒ đáng tin hơn" nhưng đơn giản hóa thành một hạng tử chỉ thị $\mathbb{1}[\text{có ảnh}]$ thay vì một mạng融合 nặng, để chạy được trên điện thoại nạn nhân.
- **Đồng thuận không gian như tín hiệu tin cậy.** Các nghiên cứu phát hiện sự kiện dựa trên đồng xuất hiện không gian–thời gian [^26] dùng số báo cáo độc lập rơi vào cùng vùng làm bằng chứng sự kiện thật. Hạng tử $\log(1+n^{corrob})$ chính là hình thức lượng hóa nhẹ của ý tưởng "corroboration" đó.
...
- **Gaussian kernel cho tương đồng không gian.** Việc dùng $\exp(-d^2/2\sigma^2)$ thay cho nghịch đảo khoảng cách tuyến tính bám theo các mô hình đồ thị địa–ngữ nghĩa (Geo-Semantic Graphs) phạt mạnh khoảng cách lớn [^46]. Đây cũng là kernel chuẩn trong spectral clustering và diffusion trên đồ thị.
...
- **Vì sao KHÔNG cộng.** Các mô hình như TwitterNews+ [^25] dựng đồ thị chỉ trên tương đồng ngữ nghĩa TF-IDF, không có ràng buộc địa lý cứng — chấp nhận được cho phát hiện chủ đề, nhưng bài toán điều phối ca nô có **bán kính hoạt động hữu hạn** nên địa lý phải là điều kiện *cần* (gate), không phải một phiếu bầu ngang hàng. Đây chính là khe hở khoa học 1 mà báo cáo lấp (thiếu thuộc tính vật lý sinh tồn trong định lượng trọng số [^13]).
- **Làm thưa đồ thị (k-NN / ngưỡng $\epsilon$).** Chuẩn trong xây dựng đồ thị tương đồng cho phân cụm phổ và phát hiện cộng đồng; đồ thị k-NN cho cấu trúc cụm rõ hơn đồ thị đầy đủ [^34].
...
- **Louvain là tiêu chuẩn vàng cho phân cụm đồ thị trọng số** [^36], độ phức tạp $\mathcal{O}(N\log N)$ chạy được thời gian thực.
...
- **Vì sao KHÔNG K-Means/DBSCAN** làm lõi: K-Means cần biết trước $K$ (bất khả thi khi không rõ bão chia thành phố thành bao nhiêu "ốc đảo"), DBSCAN nhạy tham số trong không gian đa chiều [^32]. Kết quả baseline ở trên xác nhận định tính này bằng số.
...
- **Ưu tiên dựa trên tổn thương (vulnerability-based prioritization).** Nền tảng lý luận cho hệ số khuếch đại $\mathcal{V}_{agg}$: thảm họa tác động bất bình đẳng, cần khuếch đại các nhóm yếu thế (người già, trẻ em, phụ nữ mang thai, người khuyết tật) thay vì giả định "nhu cầu đồng nhất" [^42]. Đây là khe hở khoa học 2 mà báo cáo lấp.
...
- **Đầu ra là input cho định tuyến.** Danh sách $\mathcal{P}(C_k)$ + trọng tâm cụm là đầu vào hoàn hảo cho các thuật toán định tuyến cứu hộ đa tiêu chí (A\* cost-aware, multi-commodity routing) [^41] — khép kín khe hở khoa học 3 (thiếu liên kết giữa phát hiện cộng đồng vĩ mô và định lượng ưu tiên chiến thuật).
...
| :-------------------------------------- | :----------------------------------------------------- | :------------------------------------------ |
| $C_i$ sigmoid                         | 0,38 (chữ) → 0,73 (ảnh) → 0,89 (ảnh+3 nguồn)     | SCBD/CrisisSpot[^11], event detection [^26] |
| $w_{ij}$ gating                       | cặp gần w=0,65; cặp xa 100km w≈0 (cộng: 0,42)     | Geo-Semantic Graph[^46], TwitterNews+ [^25] |
| Gating vs cộng (exp1A)                 | đường kính 100 km → 0,30 km, ARI giữ 0,892       | khe hở[^13]                                |
| Louvain/Leiden                          | ARI 0,892 vs Spectral 0,34, K-Means 0,69               | Louvain[^36], Leiden [^51]                  |
| $\widetilde{\mathcal{N}}$ chuẩn hóa | cụm 216 người hết áp đảo sau nén log           | AHP-Entropy[^46]                            |
| $\mathcal{V}_{agg}$ nhân             | cụm yếu thế được khuếch đại theo lõi rủi ro | vulnerability prioritization[^42]           |
