...
policy-valid interval $[1,2]$. Table~\ref{tab:sensitivity} will report the
resulting metric ranges once the locked rerun is complete; a constant is
considered stable if the mean metric varies by less than $0.01$ across its
valid tested levels. No sensitivity value is inferred from the existing fixed-
configuration CSVs.

\begin{table}[t]
\centering\scriptsize
\caption{Candidate-4.1 one-at-a-time sensitivity of mean NDCG@5 to fixed
heuristic constants. The metric-range cells remain pending until the locked
40-seed rerun is completed; values must be copied from its summary CSV, not
estimated.}
...
$\omega_E$ & $.34$ & component $\times\{.8,1,1.2\}$, renormalized & [FILL IN]\\
$\omega_F$ & $.33$ & component $\times\{.8,1,1.2\}$, renormalized & [FILL IN]\\
$\omega_N$ & $.33$ & component $\times\{.8,1,1.2\}$, renormalized & [FILL IN]\\
$\mu$ (vulnerability amplification) & $2$ & $\{1.6,1.8,2.0\}$ & [FILL IN]\\
...
generator/configuration metadata. The raw anchor snapshot and third-party
license record still require a provenance audit before public release.

\paragraph{Data and Code Availability.}
The analysis code, derived synthetic result tables, manifests, and figure
generation script will be released in an immutable repository or DOI after
the provenance and third-party-license audit is complete. Until that release,
reviewers may request the derived artifact bundle from the corresponding
author. The final camera-ready version will state the public URL, immutable
revision, and the separately approved code/data licenses; no public URL or
license is asserted in this revision.
